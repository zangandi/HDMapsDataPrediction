clear all
%
addpath('.\satelliteImgRetriever\')
%
load xian_hd_maps_1022.mat
load osm_1013_link.mat
load osm_1013_adjacency.mat
% load traffic
load traffic_1029.mat
osm_node_list = csvread('.\data\xian_osm_1009\nodes.csv');
bbox = [34.279936, 108.92185, 34.207309, 109.009348];
%%%%%%%%%%%%%%%%%%%%%%%%%%
save_dir = 'G:\datasets\ready_1029\';
trace_dir = 'G:\datasets\didi\data\mm\gps_20161001_1029\';
all_trace_dir = dir(strcat(trace_dir,'*.csv'));
%
next_n = 20;
%
[local_hd_maps_min, local_hd_maps_max] = localMinMax(hd_maps(:,:,1));
% write meta data
% fid = fopen(strcat(save_dir,'run.meta'), 'w+');
% fprintf(fid, 'next n: %d\n', next_n);
% fprintf(fid, 'Maps: osm_1013_link, osm_1013_adjacency, xian_hd_maps_1022\n');
% fprintf(fid, 'Traffic: traffic_1029\n');
% fprintf(fid, 'OSM node list: xian_osm_1009\n');
% fprintf(fid, 'Trace dir: %s\n', trace_dir);
% fprintf(fid, 'Bounding Box: %.8f, %.8f, %.8f, %.8f\n', bbox);
% fclose(fid);

%
% fid = fopen(strcat(save_dir,'trace_uuid.list'), 'w+');
%
for i = 1:length(all_trace_dir)-1
    %
    trace_uuid = all_trace_dir(i).name;
    trace_uuid = trace_uuid(1:end-4);
    % load trace
    trace = csvread(strcat(trace_dir,all_trace_dir(i).name),1,0);
    %
    all_feature = [];
    % validate trace
    if ValidateTrace(trace, 3, 100, bbox)
        % get mdc
        mdc_of_poses = mdc(hd_maps, trace, bbox, osm_adjacency, osm_node_list, osm_intersection, link_id_list);
        % get internal features
        internal_feature = getInternalFeatures(trace);
        % get maps
%         next_n_maps = getMaps(hd_maps, trace, bbox, osm_adjacency, osm_node_list, osm_intersection, link_id_list, 10);
        try
            [next_n_maps, next_n_traffic_tti, next_n_traffic_speed] = ...
                getMapsAndTraffic(hd_maps, trace, bbox, osm_adjacency, osm_node_list, osm_intersection,...
                                  link_id_list, speed_sequence, tti_sequence, next_n);
            %
            % write data
            % [index, mdc, internal features .... , external features...]
            % 
            % normalization and non-linear transformation
%             mdc_of_poses_in_log = data2Log(mdc_of_poses);
             
            mdc_of_poses_normalized = normailzeData(mdc_of_poses, local_hd_maps_min, local_hd_maps_max);
            next_n_maps_normalized = normailzeData(next_n_maps, local_hd_maps_min, local_hd_maps_max);
            
            %
            %
            all_feature = cat(2, mdc_of_poses_normalized, internal_feature,...
                                 next_n_maps_normalized,...
                                 next_n_traffic_tti, next_n_traffic_speed);
            csvwrite(strcat(save_dir, trace_uuid, '.csv'), all_feature);
            % write to uuid list
            fprintf(fid, '%s\n', trace_uuid);
        catch
            disp(strcat(trace_uuid, [' error.']));
        end
    else
        disp(strcat(trace_uuid, [' pass.']));
    end
end%endfor i
% fclose(fid);
%
figure()
hold on
trace_map = zeros(size(blank_image,1), size(blank_image,2));
for i = 1:size(rest_xy,1)
    trace_map(rest_xy(i,1), rest_xy(i,2)) = 0.5;
end
figure()
hold on
% imshow(cat(3, hd_maps(:,:,1)./max(max(hd_maps(:,:,1))), trace_map, zeros(size(blank_image,1), size(blank_image,2))))
im = imread('xian_hd_maps_0602.png');
image('CData',flipdim(im,1),'XData',[108.92185 109.009348],'YData',[34.207309 34.279936])
bbox = [34.279936, 108.92185, 34.207309, 109.009348];

trace = csvread('G:\datasets\didi\data\mm\gps_20161013\00bf156ba089ed09a07a39eedb54bb31.csv',1,0)


ylim([34.207309 34.279936])
xlim([108.92185 109.009348])
osm_to_didi_mis_lon = 0.0047;
osm_to_didi_mis_lat =-0.0016;
for i = 1:size(osm,1)
    link = osm{i};
    plot(link(:,2) + osm_to_didi_mis_lon, link(:,1) + osm_to_didi_mis_lat,'k--', 'linewidth', 2);
end

for i = 1:size(trace,1)
    plot(trace(i,5), trace(i,4),'ro', 'linewidth', 2);
    plot(trace(i,2), trace(i,3),'ro', 'linewidth', 2);
    plot([trace(i,5) trace(i,2)], [trace(i,4) trace(i,3)], 'm-', 'linewidth', 2);
end

plot(trace(:,5), trace(:,4),'r-', 'linewidth', 2);
plot(trace(:,2), trace(:,3),'y-', 'linewidth', 2);


